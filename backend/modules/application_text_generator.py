...
def generate_application_text(request: dict) -> dict:
    ...
    # Strip rationale from design_taste if encoded as "name|rationale"
    raw_taste = banner_details.get("design_taste", "")
    display_taste = raw_taste.split("|")[0] if "|" in raw_taste else raw_taste
    ...
    text_content = _get_text_from_claude(response)
