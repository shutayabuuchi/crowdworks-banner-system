...
def generate_prompts(analyzed_job: dict) -> list:
    ...
    for taste in DESIGN_TASTES:
        gemini_input = GEMINI_PROMPT_TEMPLATE.format(
            banner_size=analyzed_job.get("banner_size", "1200x628"),
            purpose=analyzed_job.get("purpose", ""),
            target_audience=analyzed_job.get("target_audience", ""),
...
def _fallback_prompt(analyzed_job: dict, taste: str) -> str:
    size = analyzed_job.get("banner_size", "1200x628")
    purpose = analyzed_job.get("purpose", "product promotion")
    target = analyzed_job.get("target_audience", "general audience")
