...
GEMINI_PROMPT_TEMPLATE = """あなたはプロのバナーデザイナーです。以下のクラウドワークス案件本文を最優先で読み取り、DALL-E 3で使用するバナー生成用の英語プロンプトを作成してください。
...
要件:
1. プロンプトは英語で200〜300語
2. 案件本文に明記された商品・媒体・テーマ・文言・サイズ・納品条件を必ず反映する
3. 案件にない架空の商品、業種、旅行・美容・料理などの別テーマを混ぜない
4. 具体的な視覚的要素（色、レイアウト、フォントスタイル）を含める
5. {design_taste}のテイストを強調
6. 日本語テキストを入れる場合は、案件本文の文言だけを使う。DALL-Eが文字を崩しやすいので、文字量は少なめにする
7. 商業バナー広告として適切なクオリティを指定
8. "banner advertisement" で始める
