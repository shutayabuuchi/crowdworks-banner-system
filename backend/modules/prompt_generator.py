...
例:
 - 食品ECなら「旬の素材感を前面に・白背景でクリーン」「温かみのある家庭的なシズル感」
 - SaaSなら「ダッシュボードUIをビジュアル化・テック感」「人物×プロダクト・信頼感」
 - 美容なら「透明感・水のきらめき」「高級感・深みのある配色」
...
5. テキスト要素は最小限にする（gpt-image-1はテキスト表示が不安定）
   必要な場合のみ: 'with minimal text "[テキスト]" in clean sans-serif'
...
def generate_prompts(analyzed_job: dict) -> list:
    ...
    try:
        ...
        proposals = _parse_json_response(response.text.strip())
    except Exception as e:
        print(f"[PromptGen] Gemini error: {e}")
        proposals = _fallback_proposals(analyzed_job)

    prompts = []
    for proposal in proposals[:NUM_PROPOSALS]:
        taste_name = proposal.get("taste_name", "提案")
        taste_rationale = proposal.get("taste_rationale", "")
        # Encode rationale into design_taste (no schema change needed): "名前|理由"
        encoded_taste = f"{taste_name}|{taste_rationale}" if taste_rationale else taste_name

        prompt = {
            "job_id": job_id,
            "prompt_id": f"PR{str(uuid.uuid4())[:8].upper()}",
            "text_prompt": proposal.get("prompt", ""),
